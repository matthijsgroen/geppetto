...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadImage() {
    try {
      ...
      console.log(
        ...
        Array.from(imageDefinition.animationNames.keys())
      );
      console.log(
        "Available controls:",
        Array.from(imageDefinition.controlNames.keys())
      );
  ...
  function setupControls() {
    ...
    if (controlSliders) {
      const controls = Array.from(imageDefinition.controlNames.entries()).slice(
        0,
        7
      );
