...
const createTestScene = (
  ...
  const layers = [];
  const mutators = {
    data: new Float32Array(mutationCount * 2),
    size: mutationCount,
  };
  const mutatorParents = {
    data: new Int32Array(mutationCount),
    size: mutationCount,
  };
  ...
  const rawMutations: Record<
    string,
    { controlId: string; steps: { value: number[] }[] }
  > = {};
  ...
  for (let i = 0; i < controlCount; i++) {
    ...
    rawControls[controlId] = {
      steps: [{ value: [0, 0] }, { value: [0.5, 0.5] }, { value: [1, 1] }],
    };
  }
...
describe("dirty tracking performance", () => {
  bench(
    "idle scene - no changes (small: 5 layers, 10 mutations, 3 controls)",
    () => {
  ...
  bench(
    "idle scene - no changes (medium: 20 layers, 80 mutations, 10 controls)",
    () => {
  ...
  bench(
    "idle scene - no changes (large: 50 layers, 250 mutations, 20 controls)",
    () => {
  ...
  bench(
    "active scene - 1 control changes (small: 5 layers, 10 mutations, 3 controls)",
    () => {
      const scene = createTestScene(5, 10, 3);
      const renderControlValues = new Float32Array(scene.defaultControlValues);
      const lastControlValues = new Float32Array(renderControlValues);
  ...
  bench(
    "active scene - 1 control changes (medium: 20 layers, 80 mutations, 10 controls)",
    () => {
      const scene = createTestScene(20, 80, 10);
      const renderControlValues = new Float32Array(scene.defaultControlValues);
      const lastControlValues = new Float32Array(renderControlValues);
  ...
  bench(
    "active scene - 1 control changes (large: 50 layers, 250 mutations, 20 controls)",
    () => {
      const scene = createTestScene(50, 250, 20);
      const renderControlValues = new Float32Array(scene.defaultControlValues);
      const lastControlValues = new Float32Array(renderControlValues);
