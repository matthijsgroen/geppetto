...
import type { ControlDefinition, GeppettoImage, Hierarchy, Keyframe, Layer, MutationVector, Vec2 } from "@geppetto/types";
...
describe("runtime performance", () => {
  ...
  const createTestScene = (
    ...
    for (let i = 0; i < layerCount; i++) {
      ...
      for (let j = 0; j < mutationsPerLayer; j++) {
        ...
        mutations[mutId] = {
          type: j % 4 === 0 ? "translate" : j % 4 === 1 ? "rotate" : j % 4 === 2 ? "deform" : "opacity",
          origin: [50, 50],
          radius: j % 4 === 2 ? 30 : -1,
        } as MutationVector;
  ...
  // Prepare test scenes
  const smallScene = prepareAnimation(createTestScene(5, 2, 3), { validate: false });
  const mediumScene = prepareAnimation(createTestScene(20, 4, 10), { validate: false });
  const largeScene = prepareAnimation(createTestScene(50, 5, 20), { validate: false });

  bench("mutation recalculation - small scene (5 layers, 10 mutations, 3 controls)", () => {
  ...
  bench("mutation recalculation - medium scene (20 layers, 80 mutations, 10 controls)", () => {
  ...
  bench("mutation recalculation - large scene (50 layers, 250 mutations, 20 controls)", () => {
