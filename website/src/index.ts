...
window.addEventListener("error", (errorEvent) => {
  errorElement.appendChild(document.createTextNode(errorEvent.message));
});
...
try {
  ...
  const start = async () => {
    ...
    const renderFrame = () => {
      if (currentLooking[0] !== lookTarget[0]) {
        if (currentLooking[0] < lookTarget[0]) {
          currentLooking[0] += Math.min(
            speed,
            lookTarget[0] - currentLooking[0]
          );
        } else {
          currentLooking[0] -= Math.min(
            speed,
      ...
      if (currentLooking[1] !== lookTarget[1]) {
        if (currentLooking[1] < lookTarget[1]) {
          currentLooking[1] += Math.min(
            speed,
            lookTarget[1] - currentLooking[1]
          );
        } else {
          currentLooking[1] -= Math.min(
            speed,
  ...
  errorElement.appendChild(document.createTextNode(e.message));
}
